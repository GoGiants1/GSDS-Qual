'''
* Cycle 탐색에 있어서 가 BFS보다 memory effiecient.
    * BFS -> queue 구현, 최단경로 구할때 좋음
    *  -> stack/재귀 구현, cycle 찾을때 좋음
...
def _recursion(G:dict, start:str, visited=[]):
    if start not in visited:
        visited.append(start)
        for node in G[start]:
            _recursion(G, node, visited)
    return visited

def _stack(G:dict, start:str):
    visited = []
    to_search = deque([])
    to_search.append(start)
    while to_search:
...
if __name__ == "__main__":
    ...
    print("undirected graph  using recursion", _recursion(G_undirected, 'A'))
    print("undirected graph  using stack", _stack(G_undirected, 'A'))
    # stack이기 때문에 D->G->B 순으로 나옴.
    # print("directed graph  using recursion", _recursion(G_directed, 'A'))