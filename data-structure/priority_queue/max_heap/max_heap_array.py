"""
- keyword: heap, max_heap, pq, priority_queue, array
...
def shift_up(heap, child):
    ...
    노드 위로 올리기 (최대 힙)
    """
    while child > 0:  # 올라갈 때 index 0을 넘어서지 않도록 조심
        parent = (child - 1) // 2
        if heap[parent] < heap[child]:  # 부모가 자식보다 작으면 교환
            heap[parent], heap[child] = heap[child], heap[parent]
            child = parent  # 자식 노드가 부모 위치로 올라감
        else:
            break
...
def shift_down(heap, parent, last):
    ...
    노드 아래로 내리기 (최대 힙)
    """
    while parent < last:  # 내려갈 때 index가 넘치지 않도록 조심
        child = parent * 2 + 1
        sibling = child + 1
        if sibling < last and heap[child] < heap[sibling]:  # 더 큰 값을 child에 넣음
            child = sibling
        if child < last and heap[parent] < heap[child]:  # 부모가 자식보다 작으면 교환
            heap[parent], heap[child] = heap[child], heap[parent]
            parent = child  # 부모 노드가 자식 위치로 내려감
        else:
            break
...
def heappop(heap):
    """
    최대 힙에서 모든 원소들이 내림차순으로 정렬되어 출력됨
    """
    if not heap:
        return "Empty Heap!"
    elif len(heap) == 1:
...
if __name__ == "__main__":
    ...
    print(f"배열의 상태: {arr}\n")
    print(f"배열의 모든 원소를 최대 힙에 push한 상태: {h}\n")
    print("최대 힙에서 모든 원소를 pop한 결과:", end=" ")
    while h:
        print(heappop(h), end=" ")
    ...
    h3 = [1, 5, 6, 4, 3, 8, 7, 2]
    for h in (h1, h2, h3):
        print(f"{h}를 최대 힙 구조로 만든 결과:", end=" ")
        heapify(h)
        print(h)
        print()
